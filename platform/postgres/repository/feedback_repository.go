...
func (f *FeedbackRepository) Delete(id int32) error {
	...
	if err := f.db.Model(&feedback).Where("id = ?", id).Update("status", enum.TripPassive).Error; err != nil {
...
func (f *FeedbackRepository) GetByProductId(id int32) ([]entity.Feedback, int64, error) {
	...
	var total int64
	listQuery := f.db.Find(&productList).Where("status != ?", enum.TripPassive).Where("trip_id = ?", id)
